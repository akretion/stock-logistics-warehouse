...
class TestStockOderpointAutomaticCreation(SavepointCase):
    ...
    def test_orderpoint_create(self):
        ...
        self.assertEqual(
            len(orderpoints), 0, "Error orderpoint quantity does not match"
        )
        # Product set to create orderpoints
        self.product2 = product_obj.create(
            {"name": "Test2", "create_orderpoint": "yes", "type": "product"}
        )
        ...
        self.assertEqual(
            len(orderpoints2), 1, "Error orderpoint quantity does not match"
        )
        # Category set to create orderpoint
        self.product3 = product_obj.create(
            {"name": "Test3", "type": "product", "categ_id": self.category1.id}
        )
        ...
        self.product4 = product_obj.create(
            {"name": "Test4", "create_orderpoint": "no", "type": "product"}
        )
        ...
        self.assertEqual(
            len(orderpoints5), 0, "Error orderpoint quantity does not match"
        )
