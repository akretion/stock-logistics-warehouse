...
class StockPickingWave(models.Model):
    ...
    _description = "Wave Picking"
    _order = "name desc"

    name = fields.Char(
        string='Wave Picking Name', default='New',
        copy=False, required=True,
        help='Name of the wave picking')
    user_id = fields.Many2one(
        'res.users', string='Responsible', track_visibility='onchange',
        help='Person responsible for this wave picking')
...
class StockPicking(models.Model):
    ...
    wave_id = fields.Many2one(
        'stock.picking.wave', string='Wave Picking', oldname="wave_id",
        states={'done': [('readonly', True)], 'cancel': [('readonly', True)]},
        help='Wave associated to this picking', copy=False)
